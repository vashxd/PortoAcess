...
- **Balsas & embarcações:** cadastro de embarcações (balsa, lancha, rebocador, outras), grade de horários recorrente por dia da semana, geração automática das viagens (partidas datadas) a partir da grade (`php artisan vessels:generate-departures`, agendado diariamente), quadro de viagens com status (agendada/embarcando/encerrada/cancelada) e viagens avulsas. No registro de entrada o operador escolhe a balsa/viagem de destino — obrigatório ou opcional conforme o tipo de entrada (`vessel_selection` em Tipos de entrada).
